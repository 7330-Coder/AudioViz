...
public class PlayerController implements Initializable {
        ...
        timeSlider.setOnMouseReleased(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                if (mediaPlayer != null) {
                    ...
                    double rate = timeSlider.getValue() / timeSlider.getMax();
                    System.out.println(rate);
                    double newTime = totalTime * rate;
    ...
    private void handleReady() {
        ...
        double dDuration = FormatUtil.formatDouble(duration.toMillis(), 1);
        lengthText.setText(dDuration + "");
        Duration ct = mediaPlayer.getCurrentTime();
        double dCurrentTime = FormatUtil.formatDouble(ct.toMillis(), 1);
        currentText.setText(dCurrentTime + "");
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(FormatUtil.formatDouble(ms, 1) + "");
